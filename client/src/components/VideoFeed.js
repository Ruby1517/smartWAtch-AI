...
//   const [recordeingChunk, setRecordingChunk] = useState([]);
...
//       // Only set MediaRecorder once
//       if (!mediaRecorder && stream instanceof MediaStream) {
//         const recorder = new MediaRecorder(stream);
  
//         recorder.ondataavailable = (e) => {
//           if (e.data.size > 0) {
//             setRecordingChunk(prev => [...prev, e.data]);
//           }
//         };
  
//         recorder.onstop = () => {
//           const blob = new Blob(recordeingChunk, { type: 'video/webm' });
//           const url = URL.createObjectURL(blob);
//           const a = document.createElement('a');
//           a.href = url; // <-- FIXED typo: was `herf`
...
//               // Draw placeholder boxes for each detection (since we don’t have bbox)
...
//                 ctx.strokeStyle = 'red';
//                 ctx.lineWidth = 2;
//                 ctx.strokeRect(x1, y1, x2 - x1, y2 - y1); // offset for demo
...
//                     stopTimeout = null;
//                 }
//               } else if (isRecording && !stopTimeout) {
//                 //No person - set a timaout to stop recording in 10s
...
//                 }, 10000) //10 seconds
//               }
//             //   if(data.detections.some(det => det.label === 'person')) {
...
//                 <li key={index}>
//                   🚨 {det.label} ({(det.confidence * 100).toFixed(1)}%) -  {det.bbox}
//                 </li>
//               ))}
//             </ul>
//         </div>
